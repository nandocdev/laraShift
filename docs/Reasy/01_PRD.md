# 📝 Product Requirements Document: Reasy (MVP 1.0)

- **Proyecto:** Reasy
- **Tipo:** Producto SaaS construido sobre el framework openSaaS
- **Estado:** Definición
- **Autor(es):** Fernando Castillo <ferncastillov@outlook.com>
- **Fecha:** 2025-07-31
- **Versión:** 1.1 (alineada a openSaaS)
- **Nombres anteriores deprecados:** ReasySof, ReserveHub. El nombre oficial del producto es **Reasy**. El framework base es **openSaaS** y no debe confundirse con el producto.

## 0. Alineación con openSaaS (normativo)

Reasy es un **producto vertical de reservas** que vive **sobre** openSaaS, no dentro de su core.

- **Framework (openSaaS):** multitenancy, auth dual, billing, provisioning, RBAC, auditoría, branding, colas, RLS. Ver `docs/PROJECT_DECISIONS.md`, `docs/ARCHITECTURE_RULES.md`, `docs/PRD.md`.
- **Producto (Reasy):** negocios, sedes, servicios, recursos, horarios, reservas, clientes del negocio, disponibilidad. Lógica de dominio vertical prohibida en `Tenant/Access|Compliance|Experience|Integrations|Workspace` del core; vive en la capa de extensión del producto.
- **Stack obligatorio:** Laravel 13+, Livewire 4, FluxUI 2, TailwindCSS, PostgreSQL + RLS (`tenant_id` + `SET LOCAL` transaccional), Redis (cache, colas, locks, sesiones). Prohibidos para este producto: React/Vue/Inertia en paneles, microservicios, CQRS/Event Sourcing, Repository genérico, sharding por tenant, JWT unificado multi-rol.
- **Bilinguismo de nombres:** `business_id` en este PRD equivale a `tenant_id` de openSaaS. `Negocio` equivale a `Tenant`.
- **Billing:** Clave (redirect) primero, dLocal (Smart Fields + MIT tarjeta) segundo, Stripe diferido. Ver `docs/BILLING.md`. Los RFs de pagos de `03_Requisitos.md` se implementan vía Billing Core + capabilities (`provider × payment_method`), nunca con SDK directo en Actions.

## 1. Introducción y Propósito

Reasy es una plataforma web (SaaS) diseñada para que pequeños y medianos negocios puedan gestionar sus reservas de citas de manera eficiente y profesional. El propósito de este MVP es lanzar un producto funcional que resuelva el problema central de la gestión de agendas, permitiendo a los negocios configurar sus servicios y horarios, y a sus clientes reservar citas a través de una página pública personalizada.
...
| **🏢 Dueño/Admin de Negocio**       | El cliente principal de Reasy. Necesita configurar su negocio, servicios, horarios, empleados y gestionar todas las reservas desde un panel centralizado. |
...
1.  El **Cliente** visita `Reasy.com/b/barberia-cool`.
...
5.  Envía el formulario. El sistema crea una reserva con estado `pending` (equivalente a `pending_payment` en `03 §5 RN1` cuando hay depósito; pendiente de confirmación del dueño cuando no lo hay).
...
| ----------------------------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------- | --------- |
| **Multitenancy**              | Sistema de multitenant       | Single Database + `tenant_id` + PostgreSQL RLS (`SET LOCAL` transaccional). `business_id` = `tenant_id`. Sin sharding | Critical  |
...
| ------------------------------------------- | ------------------------------------------------------------------------------------------ | ----------- |
| **Pagos en línea**                          | Vía Billing Core openSaaS (Clave/dLocal); Stripe diferido. Ver `docs/BILLING.md` y `03 §6` | Q1 post-MVP |
...
3. **Componentes:**
    - Biblioteca de componentes reutilizables con FluxUI 2
...
- **Nota:** Prototipos de referencia. La implementación usa FluxUI 2, no Radix/Shadcn.
...
Monolito modular openSaaS (ver `docs/ARCHITECTURE_RULES.md`). Sin frontend desacoplado, sin microservicios.

- **Backend:** Laravel 13+, módulos `Platform/Central/Tenant` + capa de extensión de producto Reasy. Lógica en Actions (`final readonly class` + `execute(DTO)` con `spatie/laravel-data`). Sin Repository genérico, sin Service Layer transversal.
- **Frontend:** Livewire 4 + FluxUI 2 + TailwindCSS + Alpine.js, render server-side. Prohibidos React/Vue/Inertia en paneles.
- **Patrón de Arquitectura:** Actions + DTOs + Contracts (`Platform/Contracts`) + Domain Events. Comunicación entre módulos solo vía Actions públicas, Contracts o Events. Nunca `Model::find()` cruzado.
...
| --------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------- |
| **Frontend**                | **Livewire 4 + FluxUI 2 + TailwindCSS + Alpine.js**               | Stack oficial openSaaS, SSR, sin SPA                      |
| **Backend API**             | **Laravel 13+ (monolito modular)**                                | Framework + Actions + DTOs `spatie/laravel-data`          |
| **Puente Backend-Frontend** | **Livewire (server-driven)**                                      | Sin Inertia/React en paneles                              |
| **Base de datos principal** | **PostgreSQL + RLS**                                              | `tenant_id`, `SET LOCAL`, JSONB, UUID                     |
| **Caching**                 | **Redis**                                                         | Cache `tenant:{id}:*`, locks, rate-limit por tenant       |
| **Autenticación**           | **Dual guard: `central` + `tenant`**                              | `CentralUser` vs `User`, sin JWT unificado ni tabla única |
| **Multitenancy**            | **Single DB + RLS openSaaS**                                      | TenantContext `scoped()`, `SET LOCAL` transaccional       |
| **Componentes UI**          | **FluxUI 2**                                                      | Accesibilidad, consistencia openSaaS                      |
| **Calendario de reservas**  | **Livewire + Flux calendar**                                      | Motor de disponibilidad en Actions, no en JS              |
| **Email & notificaciones**  | **Laravel Notifications + Queue Redis + SES/SMTP tenant**         | Jobs `TenantAware`, idempotentes                          |
| **Tareas en segundo plano** | **Laravel Queues (Redis/Horizon) + Jobs TenantAware**             | `RehydrateTenantContext`, reintentables                   |
| **API Docs**                | **Scribe (solo si hay API pública de producto)**                  | No para paneles Livewire                                  |
| **Contenedores**            | **Docker + Docker Compose**                                       | Entornos de desarrollo consistentes y fácil despliegue    |
| **Despliegue**              | **Railway / Docker (actual) — K8s solo post-MVP si se justifica** | Sin microservicios                                        |
| **Testing**                 | **Pest (PHP) sobre PG real para RLS + SQLite para suite rápida**  | `CrossTenantLeakTest` obligatorio en módulos tenant-aware |
| **Análisis estático**       | **Pint + Larastan**                                               | Calidad de código openSaaS                                |
...
1. **Integración de Pagos en Línea:**
    - Vía Billing Core openSaaS: Clave (redirect) primero, dLocal (Smart Fields + MIT tarjeta) segundo, Stripe diferido hasta demanda real. Ver `docs/BILLING.md`. Prohibido SDK directo en Actions.
...
# 📜 Normativas de Desarrollo y Control de Versiones: Reasy
...
4.  **Sigue las Guías del Framework:** Adhiérete a las convenciones y mejores prácticas de Laravel y Livewire/Flux (ver `docs/ARCHITECTURE_RULES.md`). No reinventes la rueda a menos que sea estrictamente necesario y esté justificado.
...
| **Desarrollo MVP - UI**   | 4 semanas | Interfaces de usuario y experiencia      | Paneles Livewire + FluxUI, integración Actions, componentes |
