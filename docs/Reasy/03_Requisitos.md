...
- [~] **RF5.1** - El administrador debe poder configurar integraciones de pago globales (vía `.env`: `config/clave.php`, `config/dlocal.php`, `config/billing.php`; secretos nunca en DB/UI por seguridad — sin pantalla dedicada por decisión explícita)
- [ ] **RF5.2** - El sistema debe permitir configurar plantillas de email transaccionales por defecto (diferido: motor de plantillas editable es un subsistema nuevo sin consumidor; las plantillas son vistas blade versionadas por deploy)
- [x] **RF5.3** - El administrador debe poder establecer políticas globales de la plataforma (`Settings/Livewire/ManagePolicies` + `PlatformPolicies`: umbral antifraude, email SecOps, ventana stale — con efecto inmediato y `activity('settings')`)

#### RF6 - Comunicación con Tenants

- [x] **RF6.1** - El sistema debe permitir enviar notificaciones a todos los administradores de negocio (`Support/Livewire/BroadcastCenter` + `SendBroadcastAction`: audiencia all/status/plan/selected, email encolado por chunks + banner; gate `broadcasts:manage` solo global admin)
- [x] **RF6.2** - El administrador debe poder crear anuncios de mantenimiento o nuevas funcionalidades (borradores, programados con `broadcasts:dispatch-due`, publicación bajo demanda, `unschedule`; `GlobalAnnouncements` en el tenant)
- [x] **RF6.3** - El sistema debe mantener un historial de comunicaciones enviadas (lista paginada + `recipient_count` + `activity('support')` por cada transición)

#### RF7 - Compliance y Auditoría

- [~] **RF7.1** - El sistema debe generar reportes de compliance (GDPR, PCI-DSS) para todos los tenants (base real: portabilidad vía `DataExport`, retención con `compliance:prune-audit-logs` —365d vía `config/compliance.php`—, trails central+tenant; reporte formal con template regulatorio diferido sin consumidor)
- [x] **RF7.2** - El administrador debe tener acceso a logs de auditoría centralizados con búsqueda y filtrado
- [x] **RF7.3** - El sistema debe proporcionar herramientas para gestionar solicitudes de eliminación de datos (flujo staff `TenantList::delete` → `PurgeTenantJob` → `PurgeTenantDataAction`: 30 tablas tenant verificadas con `PurgeCompletenessTest`, storage y rate-limits incluidos)

#### RF8 - Monitoreo de Salud del Sistema

- [x] **RF8.1** - El dashboard debe mostrar métricas de infraestructura y performance en tiempo real (`Operations/Livewire/HealthMonitor`: plataforma, tenants por salud, incidentes + JSON `/up/*` para sondas)
- [x] **RF8.2** - El sistema debe generar alertas automáticas para incidentes críticos (`operations:alert-incidents` horario: `DetectCriticalIncidents` compartida con el monitor + mail a global admins con dedupe 24h por incidente)
- [x] **RF8.3** - El administrador debe poder ejecutar maintenance mode por tenant o globalmente (`maintenance_mode`/`read_only` por tenant vía `ManageTenant` + `EnsureTenantIsActive`; global vía `php artisan down` nativo)

#### RF9 - Gestión de Integraciones

- [x] **RF9.1** - El sistema debe permitir gestionar integraciones de terceros centralizadamente (SMTP/API keys/webhooks por tenant + integraciones de plataforma vía `.env`; central sin acceso cruzado por diseño)
- [x] **RF9.2** - El administrador debe poder configurar webhooks para partners y desarrolladores (`Integrations/Livewire/ManageWebhooks`: URL HTTPS, eventos, secreto `whsec_` único + firma HMAC-SHA256, reintentos con backoff, historial de deliveries)
- [x] **RF9.3** - El sistema debe gestionar API keys y rate limits por tenant (`Access/Livewire/ManageApiKeys` + `GenerateApiKey` con scopes/quotas + `AuthenticateApiKey`; `TenantRateLimiter` + `ApplyTenantRateLimits`)

#### RF10 - Análisis Predictivo

- [~] **RF10.1** - El sistema debe identificar tenants en riesgo de churn usando algoritmos predictivos (v1 heurística explicable en `Operations/Queries/TenantRiskInsights`: mora + reintentos + inactividad de pago, con razones; ML diferido hasta volumen de datos que lo justifique)
- [x] **RF10.2** - El dashboard debe mostrar análisis de patrones de uso para optimización (`quotaSaturation` por métrica + candidatos con % de uso visible en el dashboard)
- [x] **RF10.3** - El sistema debe generar recomendaciones automáticas de upgrade de plan (saturación ≥80% de quota con plan superior disponible → plan sugerido)
...
- [x] **RF12.3** - El sistema debe permitir transferir recursos entre sedes (vía reasignación de `location_id` en `ManageResources` —acción "Edit / Transfer"— con validación same-tenant y auditoría `resource.updated`; test de transfer + rechazo cross-tenant en `ResourceManagementTest`)
...
- [x] **RF14.1** - El sistema debe permitir gestionar personal, salas y equipamiento como recursos (`Product/Resources`: `ResourceType staff|room|equipment` + `ManageResources`, merge en `develop`, `ResourceManagementTest` 8/8)
- [x] **RF14.2** - El administrador debe poder asignar servicios específicos a cada recurso (pivot `resource_service` con `tenant_id` + RLS; validación same-tenant en regla y en Action)
- [x] **RF14.3** - El sistema debe permitir definir habilidades y especialidades por recurso (`skills json` con filas dinámicas en UI)
- [x] **RF14.4** - El administrador debe poder establecer tarifas diferentes por recurso (`rate_cents` nullable + `currency`: null = precio del servicio; la precedencia se ejecuta en el motor de reservas)
...
| RF5 config global      | ✅                     | `Settings/Livewire/ManagePolicies` + `PlatformPolicies` (umbral fraude, SecOps, stale; gate `policies:manage`, `activity`), `CentralBranding`/`PlatformBranding`; pagos vía `.env`, plantillas blade versionadas                                                                                                                                                |
| RF6 broadcasts         | ✅                     | `Support/Livewire/BroadcastCenter` (gates `broadcasts:view/manage`), `SendBroadcastAction` + `broadcasts:dispatch-due`, audiencia all/status/plan/selected, historial + `activity('support')`                                                                                                                                                                   |
| RF7 compliance central | ✅                     | `Support/Livewire/CentralAuditLog` (filtros search/log/tenant/fechas, detalle, export CSV) + `Tenant/Compliance/AuditLogViewer` con gates                                                                                                                                                                                                                       |
| RF8 health             | ✅                     | `Operations/Livewire/HealthMonitor` (UI + incidentes + ack) + `operations:alert-incidents` (mail crítico con dedupe) + JSON `/up/*`; `maintenance_mode`/`read_only` por tenant + `down` global                                                                                                                                                                  |
| RF9 integraciones      | ✅                     | `Access/Livewire/ManageApiKeys`, `GenerateApiKey`, rate limits ✅; `Integrations/Livewire/ManageWebhooks` + HMAC + backoff + deliveries ✅; portal docs/sandbox ❌                                                                                                                                                                                              |
| RF10 predictivo        | 🟡/✅                  | `Operations/Queries/TenantRiskInsights` (churn heurístico con razones, candidatos upgrade, saturación) en dashboard; ML diferido                                                                                                                                                                                                                                |
...
| RF58 audit trail       | ✅ tenant / ✅ central | `AuditLogViewer` + `DataExport` (tenant) y `CentralAuditLog` + export CSV (central)                                                                                                                                                                                                                                                                             |
| RF59 privacidad        | ✅                     | Export + `PurgeTenantDataAction` (30 tablas, `PurgeCompletenessTest`) + retención `compliance:prune-audit-logs`; anonimización parcial pendiente                                                                                                                                                                                                                |
...
| RF12 sedes             | ✅/🟡/⬜    | `Product/Locations` en `develop` (merge `4aa1544`): RF12.1 ✅ `ManageLocations` + `LocationManagementTest` 7/7; RF12.2 🟡 fundación (`timezone`/`settings` por sede, pivot con RF13); RF12.3 ✅ transfer vía `ManageResources` (RF14) |
...
| RF14 recursos          | ✅          | `Product/Resources` en `develop`: `Resource` (`staff/room/equipment`, `location_id` nullable, `skills`, `rate_cents`, pivot `resource_service` con RLS) + `ManageResources`, `ResourceManagementTest` 8/8                             |
