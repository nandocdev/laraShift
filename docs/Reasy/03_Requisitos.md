...
> **Versión:** 2.2 (alineada a openSaaS)
...
- **RF21.1** - El administrador debe poder operar con los proveedores soportados por el Billing Core: Clave (redirect) y dLocal (Smart Fields + MIT tarjeta). Stripe queda diferido hasta demanda real (`docs/BILLING.md §16`). Prohibido conectar SDKs de pago directamente desde Actions.
- **RF21.2** - El sistema debe permitir configurar múltiples métodos de pago solo entre los que el proveedor declara vía `supports(capability, forMethod)` (p. ej. dLocal tarjeta sí admite recurrencia; efectivo/voucher no)
...
| ---------------------------------------------------------------------------------------------------------------------------- | ---------------- | ------------------------------------------------------------------------------- | --------------------------------------------- | ------------------------------------------------------------- |
| RF1–RF10 (plataforma: tenants, planes, dashboard, facturación, config, comms, compliance, health, integraciones, predictivo) | Framework        | `Central/Provisioning, Catalog, Billing, Settings, Support, Operations, Growth` | PA-01/02                                      | Heredado del framework; Reasy no los reimplementa             |
| RF11–RF16 (negocio, sedes, servicios, recursos, horarios, empleados)                                                         | Producto         | Extensión Reasy sobre `Tenant/Access + Experience` (scaffolding)                | BO-01/02/05/06/07 hechos; BO-03/04 pendientes | M: BO-03/04                                                   |
