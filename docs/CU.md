...
| **Feature Flags**  | `laravel/pennant`           | Paquete oficial                                  | Gating de funcionalidades por Tenant                     |
...
├── Central/    → Administración del Producto: Auth administrativa, Provisioning
...
├── Infrastructure/   → Persistence (Repositorios), ExternalServices (APIs), Adapters
...
2. **Sin fuga de modelos Eloquent:** Está terminantemente prohibido importar `Domain/Models/` de un módulo en otro. Si un módulo requiere validar datos de otro módulo de `Central`, debe consultar una interfaz en `Platform/Contracts/` o consumir un DTO expuesto por una `Action`.
...
- **Precondición:** `CreateTenantData` validado (name, slug, email). El aprovisionamiento se activa directamente tras el registro.
- **Flujo Principal:**
...
* **Postcondición:** El tenant queda en `status = active`, con subdominio reservado, roles base creados, usuario admin notificado y actividad auditada en `activity_log`.
...
- **UC-C-03: Suspensión Preventiva por Infracción de Términos**
- **Estado: ✅ IMPLEMENTADO**
- **Actor:** Agente de Plataforma.
- **Precondición:** Abuso flagrante de cuota o violación de términos del servicio.
- **Flujo Principal:**
    1. El agente actualiza el campo `status` del tenant a `suspended` y se registra en `activity_log`.
    ...
    4. Si la ruta interceptada no es de login, el middleware rechaza la petición con un `abort(403)` (solo login permitido).
- **Postcondición:** Todo request a base de datos de dicho tenant (fuera de login) es rechazado en la capa de middleware. No se requiere purgar sesiones de la BD ni invalidar cachés complejos, ya que la barrera del middleware neutraliza efectivamente cualquier sesión abierta.
...
- **Flujo Principal:**
    1. El agente selecciona el tenant en el panel Host (`TenantList` Livewire) y solicita una sesión de soporte llenando el _Ticket ID_ obligatorio y una _justificación textual_ (mínimo 20 caracteres).
...
- **Flujo Principal:**
    1. El `TenantRateLimiter` aplica el límite de requests por minuto configurado para el tenant activo.
...
- **Precondición:** Tenant que requiere RBAC avanzado o Custom Roles.
- **Flujo Principal:**

1. El administrador ingresa al submódulo de roles del workspace.
2. Crea un nuevo perfil (ej. "Auditor Financiero") y selecciona permisos booleanos específicos (`team.read`, `audit.read`, `documents.read`).
...
- **Precondición:** El cliente posee un Identity Provider (IdP) como Okta o Microsoft Entra ID.
- **Flujo Principal:**
...
- **Actor:** Tenant Owner.
- **Precondición:** El usuario es el titular absoluto del workspace.
- **Flujo Principal:**

1. El Owner accede al área de peligro (_Danger Zone_) y solicita la rescisión del servicio.
...
4. El sistema bloquea nuevos registros y marca el workspace con `deleted_at = NOW()`.
...
- **Alcance:** Recuperación de cuenta por reseteo seguro de contraseña vía email, enrolamiento y desafío de MFA (TOTP / WebAuthn), generación y consumo de códigos de respaldo (_backup codes_), y rotación de credenciales con revocación simultánea de tokens de sesión activos.
