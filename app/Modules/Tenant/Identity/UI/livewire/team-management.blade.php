<div class="flex flex-col gap-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <flux:subheading>{{ __('Manage users, roles and invitations for :tenant.', ['tenant' => tenant('name')]) }}
            </flux:subheading>
        </div>
                    ...
                    <flux:table.row :key="$member->id">
                        <flux:table.cell class="flex items-center gap-3">
                            ...
                            <div class="flex flex-col">
                                <span
                                    class="font-medium text-sm text-zinc-900 dark:text-white">{{ $member->name }}</span>
                                <span class="text-xs text-zinc-500">{{ $member->email }}</span>
                            </div>
                        </flux:table.cell>
                        <flux:table.cell>
                            <flux:badge size="sm" variant="outline">
                                {{ strtoupper($member->getRoleNames()->first() ?: 'member') }}</flux:badge>
                        </flux:table.cell>
                        <flux:table.cell>
                            @if ($member->is_active)
                    ...
                    <flux:table.rows>
                        @foreach ($invitations as $invite)
                            <flux:table.row :key="$invite->id">
                                ...
                                <flux:table.cell>
                                    <flux:badge size="sm" variant="outline">{{ strtoupper($invite->role->name) }}
                                    </flux:badge>
                                </flux:table.cell>
                                <flux:table.cell>
                                    @if ($invite->expires_at->isPast())
                                        <div class="flex flex-col">
                                            <flux:badge size="sm" variant="danger">{{ __('EXPIRED') }}
                                            </flux:badge>
                                            <span
                                                class="text-[10px] text-zinc-500 mt-1">{{ $invite->expires_at->format('Y-m-d H:i') }}</span>
                                        </div>
                                    @else
