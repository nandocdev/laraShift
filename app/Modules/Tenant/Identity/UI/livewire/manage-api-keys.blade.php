<div class="flex flex-col gap-6 py-12">
    <div class="flex items-center justify-between">
        <div>
            ...
            <flux:subheading>{{ __('Generate secure keys to integrate your tenant data with external systems.') }}
            </flux:subheading>
        </div>
                        ...
                        <flux:table.cell>
                            <div class="flex flex-wrap gap-1">
                                @foreach ($key->scopes as $scope)
                                    <flux:badge size="sm" variant="outline" class="text-[10px]">
                                        {{ $scope }}</flux:badge>
                                @endforeach
                            </div>
                        </flux:table.cell>
    ...
    <flux:modal name="generate-key" class="min-w-[25rem]">
        <form wire:submit="generate" class="space-y-6">
            ...
            <flux:input wire:model="name" :label="__('Key Name')" placeholder="{{ __('External ERP Integration') }}"
                required />

            <div class="space-y-2">
                <flux:label>{{ __('Scopes') }}</flux:label>
                <div
                    class="grid grid-cols-1 gap-2 p-3 border border-zinc-200 dark:border-zinc-700 rounded-lg max-h-[200px] overflow-y-auto">
                    @foreach ($availableScopes as $key => $label)
                        <label
                            class="flex items-center gap-3 p-2 hover:bg-zinc-50 dark:hover:bg-zinc-800 rounded cursor-pointer transition-colors">
                            <input type="checkbox" wire:model="selectedScopes" value="{{ $key }}"
                                class="rounded border-zinc-300 text-indigo-600 focus:ring-indigo-600">
    ...
    <flux:modal name="show-key" :open="$showingKey" class="min-w-[30rem]">
        <div class="space-y-6 text-center">
            <div>
                ...
                <flux:text class="text-red-500 font-bold">
                    {{ __('IMPORTANT: Copy this key now. It will never be shown again.') }}</flux:text>
            </div>

            <div
                class="p-4 bg-zinc-100 dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 break-all font-mono text-sm select-all">
                {{ $plainKey }}
            </div>

            <div class="flex justify-center">
                <flux:button wire:click="closeKeyModal" variant="primary">{{ __('I have saved the key') }}
                </flux:button>
            </div>
        </div>
    </flux:modal>
