<div class="flex flex-col gap-6 py-12">
                        ...
                        <flux:table.cell>
                            <div class="flex flex-wrap gap-1">
                                @forelse($role->permissions as $perm)
                                    <flux:badge size="sm" variant="outline" class="text-[10px]">{{ $perm->name }}
                                    </flux:badge>
                                @empty
                                    <span
                                        class="text-xs text-zinc-400 italic">{{ __('No specific permissions') }}</span>
                                @endforelse
                            </div>
                        </flux:table.cell>
                        <flux:table.cell>
                                ...
                                <flux:menu>
                                    <flux:modal.trigger name="edit-role">
                                        <flux:menu.item icon="pencil" wire:click="edit('{{ $role->id }}')">
                                            {{ __('Edit Permissions') }}</flux:menu.item>
                                    </flux:modal.trigger>
                                    ...
                                    @if (!$role->is_system)
                                        ...
                                        <flux:menu.item variant="danger" icon="trash"
                                            ...
                                            wire:confirm="{{ __('Are you sure? This action is permanent.') }}">
                                            {{ __('Delete Role') }}
                                        </flux:menu.item>
                                    @endif
                                </flux:menu>
    ...
    <flux:modal name="edit-role" class="min-w-[30rem]">
        <form wire:submit="update" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Edit Role: :name', ['name' => $editingRole?->name]) }}
                </flux:heading>
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-cols-1 gap-2 p-3 border border-zinc-200 dark:border-zinc-700 rounded-lg">
                    @foreach ($availablePermissions as $key => $label)
                        <label
                            class="flex items-center gap-3 p-2 hover:bg-zinc-50 dark:hover:bg-zinc-800 rounded cursor-pointer transition-colors">
                            <input type="checkbox" wire:model="editPermissions" value="{{ $key }}"
                                class="rounded border-zinc-300 text-indigo-600 focus:ring-indigo-600">
                            <div class="flex flex-col">
                                <span class="text-xs font-bold font-mono">{{ $key }}</span>
                                <span class="text-[10px] text-zinc-500">{{ $label }}</span>
