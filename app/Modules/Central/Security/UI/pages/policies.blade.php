<div class="flex flex-col gap-6 py-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <flux:card class="space-y-3">
            ...
            <flux:text class="text-3xl font-bold">{{ number_format($totalActiveKeys) }}</flux:text>
        ...
        <flux:card class="space-y-3">
            ...
            <flux:text class="text-3xl font-bold">{{ number_format($keysNearingRotation) }}</flux:text>
        ...
        <flux:card class="space-y-3">
            ...
            <flux:text class="text-3xl font-bold">{{ number_format($activeTenants) }}</flux:text>
            ...
            <flux:table.rows>
                @forelse($plans as $plan)
                    ...
                    <flux:table.row>
                        ...
                        <flux:table.cell>{{ ($enc['encrypt_at_rest'] ?? true) ? __('Yes') : __('No') }}</flux:table.cell>
                        <flux:table.cell>{{ $ret['audit_logs'] ?? 365 }}d</flux:table.cell>
                        <flux:table.cell>{{ $planKeyCounts[$plan->slug] ?? 0 }}</flux:table.cell>
                    </flux:table.row>
                @empty
                    <flux:table.row>
        ...
        <div class="max-h-64 overflow-y-auto space-y-1">
            @forelse($recentEvents as $event)
                <div class="flex items-center justify-between text-xs py-1 border-b last:border-0">
                    ...
                    <span class="text-zinc-400">{{ $event->causer?->name ?? __('System') }}</span>
                </div>
            @empty
